...
{
    var t = ds_map_find_value(async_load, "type"), sock;
    ...
    switch (t)
    {
        ...
        case 2:
            ...
            with (obj_onlinePlayer)
            {
                if (socket == sock)
                {
                    obj_server.create_notifAsync(string("{0} has disconnected!", name), 65535);
                    instance_destroy();
                    break;
                }
            }
...
if (ds_map_find_value(async_load, "type") == 3)
{
    ...
    if (buffer_exists(buff))
    {
        ...
        switch (q.type)
        {
            case networkpacket.notification:
                ds_list_add(notifs, q);
                break;
            
            case networkpacket.playerlist:
            ...
            case networkpacket.particle:
                if (room == q.room)
                    ds_list_add(obj_particleSystem.particles, q.particle);
                
                break;
            
            case networkpacket.disconnect:
            ...
            case networkpacket.afterimage:
                if (room == q.room)
                    ds_list_add(obj_afterimageSystem.afterimages, q.after);
                
                break;
            
            case networkpacket.sound:
                if (room == q.room)
                {
